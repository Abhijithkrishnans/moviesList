...
enum MLUtilities {
    static func getDecodedBundleData<T:Codable>(WithName name:String, ext:String, type:T.Type) -> T? {
    ...
    static func getDataFromBundle(WithName name:String, ext:String) -> Data? {
    ...
    static func getDecodedData<T:Codable>(type:T.Type, data:Data) -> T? {
        let decoder = JSONDecoder()
        let loadedDataModel:T?
        loadedDataModel = try? decoder.decode(type.self, from: data)
        return loadedDataModel
...
extension MLUtilities {
    ...
    static func getStringFromDate(datestring:NSDate,dateformat:String)-> String    {
